...
type StatusToolbarData = StyleAndTextTuples
...
def _get_stats_bar(queue: JobQueueContents) -> tuple[StatusToolbarData, int]:
...
def _handle_run_options(toolbar_stats: StatusToolbarData, pending_count: int, use_advanced_sushi_args: bool = False) -> None:
...
def _handle_merge_options(toolbar_stats: StatusToolbarData) -> None:
    ...
    match merge_choice:
        case 1:
            ...
            if confirm_prompt.get(bottom_toolbar=toolbar_stats):
                QueueExecutionService.merge_completed_video_jobs(selected_jobs, parent_queue=qm.main_queue)
        case 2:
...
def show_main_queue() -> None:
    ...
    while True:
        ...
        validations: dict[str, bool] = {
            "enable_advanced_sushi_args": bool(s.config.sync_workflow["enable_sushi_advanced_args"]),
            "can_merge": bool(
